...
c.execute("""CREATE TABLE IF NOT EXISTS Freeuserdata (
        id int AUTO_INCREMENT,
        freeuserid int,
        phone_no text
   )""")
...
def alredyuser():
    ...
    dd = fetch_UserData_table()
    alredy = dd
    amf = []
    for g in alredy:
        alredy_user = (g[1])
        amf.append(alredy_user)
...
def srftimer(phone_no,userid):
    c.execute("UPDATE Freeuserdata SET phone_no = ? WHERE freeuserid = ?",(phone_no, userid))
    # c.execute("INSERT INTO UserData VALUES (?)", (phone_no,))
    conn.commit()


def srffetch_timer(userid):
    c.execute(f"SELECT phone_no FROM Freeuserdata WHERE freeuserid ='{userid}'")
    try:
        phonenumber = int(c.fetchone()[0])
        conn.commit()
        return phonenumber
    except:
        return int(1672555796)
    

def create_free(user_id):
    sql = f"Insert into Freeuserdata (freeuserid) values({user_id})"
    c.execute(sql)
    conn.commit()
...
def fetch_Freeuser_table():
    c.execute("SELECT * FROM Freeuserdata")
    table = c.fetchall()
    conn.commit()
    return table


def delete_alldata_Freeuser():
    c.execute("DELETE FROM Freeuserdata")
    conn.commit()



def delete_specific_user(user_id):
    c.execute("DELETE FROM Freeuserdata WHERE freeuserid = ?", (user_id,))
    conn.commit()
        

def freeuser_fetcher():
    c.execute("SELECT freeuserid FROM Freeuserdata")
    list = c.fetchall()
    conn.commit()
    return list


def check_Freeuser(id):
    list = freeuser_fetcher()
    for x in list:
        if id in x:
            return True
        else:
